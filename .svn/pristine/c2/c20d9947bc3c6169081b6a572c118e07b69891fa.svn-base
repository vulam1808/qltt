...
class Model_DocumentBase{
    ...
    public function setType_Document($value){
        $this->_type_document_id=$value;
        return $this;
    }
    public function getType_Document(){return $this->_type_document_id;}
}
class Model_DbTable_Document extends Zend_Db_Table_Abstract{
    protected $_name="document";
    protected $_primary="id";
}
class Model_DocumentMapperBase extends BaseMapper{
    protected $_dbTable;
    ...
    public function getDbTable()
    {
        if (NULL === $this->_dbTable) {
            $this->setDbTable("Model_DbTable_Document");
        }
        return $this->_dbTable;
    }
    public function save(Model_Document $entry){
        $data = array(
                'id' => $entry->getId(),
                'code' => $entry->getCode(),
                'name' => $entry->getName(), 
                'type_document_id'=>$entry->getType_Document(),
                'file'=>$entry->getFile(),
...
//                'status'=>$entry->getStatus(),
    ...
    public function find($id, Model_Document $entry){
        ...
        $entry->setId($row->id)
                ->setId($row->id)
                ->setCode($row->code)
                ->setType_Document($row->type_document_id)
                ->setName($row->name)
                ->setFile($row->file)
