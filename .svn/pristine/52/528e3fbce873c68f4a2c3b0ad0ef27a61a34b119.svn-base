 ...
 */
class Model_Master_Business_TypeBase{
    protected $_id;
    protected $_name;  
    protected $_code;    
...
class Model_DbTable_Master_Business_Type extends Zend_Db_Table_Abstract
{
    protected $_name = "master_business_type";
    protected $_primary = "id";
}
class Model_Master_Business_TypeMapperBase extends BaseMapper{
     protected $_dbTable;
    ...
    public function getDbTable()
    {
        if (NULL === $this->_dbTable) {
            $this->setDbTable("Model_DbTable_Master_Business_Type");
        }
        return $this->_dbTable;
    }
    
    public function save(Model_Master_Business_Type $entry){
        $data = array(
                'id' => $entry->getId(),
                'name'=> $entry->getName(),  
                'code'=>$entry->getCode(),
    ...
    public function find($id,  Model_Master_Business_Type $entry){
     ...
     public function fetchPaging($page, $pageSize, $orderBy, $orderDirection, &$totalRecord, $filters=null)
    {
		...
		$queryTotal = "SELECT count(*) as totals FROM master_business_type ".$sFilter."";
		$totalRecord = $this->getTotalRecords($queryTotal, $aFilterValues);
		$query = "SELECT * FROM master_business_type ".$sFilter." order by ".$orderBy." ".$orderDirection." limit ".($page-1)*$pageSize.",".$pageSize."";
