 ...
 */
class Model_Master_Items_ConditionBase{
    protected $_id;
    protected $_name;  
    protected $_code;    
...
class Model_DbTable_Master_Items_Condition extends Zend_Db_Table_Abstract
{
    protected $_name = "master_items_condition";
    protected $_primary = "id";
}
class Model_Master_Items_ConditionMapperBase extends BaseMapper{
     protected $_dbTable;
    ...
    public function getDbTable()
    {
        if (NULL === $this->_dbTable) {
            $this->setDbTable("Model_DbTable_Master_Items_Condition");
        }
        return $this->_dbTable;
    }
    
    public function save(Model_Master_Items_Condition $entry){
        $data = array(
                'id' => $entry->getId(),
                'name'=> $entry->getName(),  
                'code'=>$entry->getCode(),
    ...
    public function find($id,  Model_Master_Items_Condition $entry){
     ...
     public function fetchPaging($page, $pageSize, $orderBy, $orderDirection, &$totalRecord, $filters=null)
    {
		...
		$queryTotal = "SELECT count(*) as totals FROM Master_Items_Condition ".$sFilter."";
		$totalRecord = $this->getTotalRecords($queryTotal, $aFilterValues);
		$query = "SELECT * FROM Master_Items_Condition ".$sFilter." order by ".$orderBy." ".$orderDirection." limit ".($page-1)*$pageSize.",".$pageSize."";
