 ...
 */
class Model_Master_ItemsBase{
    protected $_id;
    protected $_name;  
    protected $_code;    
    protected $_type_commodity;
    ...
    {   
        ...
        if (("mapper" == $name) || !method_exists($this, $method)) {   
            throw new Exception("Invalid Master_Items_Condition");   
        }   
        $this->$method($value);   
    }   
    ...
    public function getCode(){return $this->_code;} 
     public function setType_Commodity($value){
        $this->_type_commodity=$value;
        return $this;
    }
    public function getType_Commodity(){return $this->_type_commodity;} 
...
class Model_DbTable_Master_Items extends Zend_Db_Table_Abstract
{
    protected $_name = "master_items";
    protected $_primary = "id";
}
class Model_Master_ItemsMapperBase extends BaseMapper{
     protected $_dbTable;
    public function setDbTable($dbTable)
    {
        ...
        if (!$dbTable instanceof Zend_Db_Table_Abstract) {
            throw new Exception("Invalid table data gateway Master_Items_Conditioned");
        }
    ...
    public function getDbTable()
    {
        if (NULL === $this->_dbTable) {
            $this->setDbTable("Model_DbTable_Master_Items");
        }
        return $this->_dbTable;
    }
    
    public function save(Model_Master_Items $entry){
        $data = array(
                'id' => $entry->getId(),
                'name'=> $entry->getName(),  
                'code'=>$entry->getCode(),
                'type_commodity'=>$entry->getType_Commodity(),
    ...
    public function find($id,  Model_Master_Items $entry){
     ...
     public function fetchPaging($page, $pageSize, $orderBy, $orderDirection, &$totalRecord, $filters=null)
    {
		...
		$queryTotal = "SELECT count(*) as totals FROM Master_Items ".$sFilter."";
		$totalRecord = $this->getTotalRecords($queryTotal, $aFilterValues);
		$query = "SELECT * FROM Master_Items ".$sFilter." order by ".$orderBy." ".$orderDirection." limit ".($page-1)*$pageSize.",".$pageSize."";
