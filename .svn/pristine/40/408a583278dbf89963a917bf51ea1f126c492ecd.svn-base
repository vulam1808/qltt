...
class Master_MasterDistrictController extends Zend_Controller_Action {
    ...
    public function addAction() {
        if ($this->getRequest()->isPost()) {   
            $redirectUrl = $this->aConfig["site"]["url"] . "master/masterdistrict/list";
            if (isset($_POST["id"])) {
    ...
    public function deleteAction(){
      ...
      public function excelAction(){
        $this->_helper->layout->disableLayout();
       //  $redirectUrl=$this->aConfig["site"]["url"]."master/district/add";
        ...
        if ($this->getRequest()->isPost()) {            
         ...
         ///
           foreach ($this->modelBCNKN->fetchAll_NKN() as $items) {               
            ...
            $objWriter = new PHPExcel_Writer_Excel5($objPHPExcel);
            $objWriter->save('php://output');
             //$redirectUrl=$this->aConfig["site"]["url"]."master/district/list";
             //$this->_redirect($redirectUrl);
            exit();
        }
    }
