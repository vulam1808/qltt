...
abstract class Zend_Db_Adapter_Abstract
{
    ...
    public function query($sql, $bind = array())
    {
     ...
     //$table[] = array();
        ...
        if ($sql instanceof Zend_Db_Select) {
            ...
            //$table = $sql->getAdapter();
            $sql = $sql->assemble();
        }
        //Kiem tra is_delete
        //$sql = strtolower($sql);
        if((strpos($sql,'select')!== false || strpos($sql,'SELECT')!== false)&& strpos($sql,'is_delete')== false)
        {
            //Cat chuỗi
            $strArray = explode(" ",$sql);
            if(strpos($sql,'where')!== false || strpos($sql,'WHERE')!== false)
            {
                    //var_dump(in_array('freetuts.net', $haystackarray));
                    for ($i = 0; $i < count($strArray) ; $i ++)
                    {
                        if($strArray[$i] == 'from' || $strArray[$i] == 'FROM')
                        {
                            $strtable = $strArray[$i+1];
                            if(strpos($strtable,',')!== false)
                            {
                                $tbl = explode(",",$strtable);
                                 for ($f = 0; $f < count($tbl) ; $f ++)
                                { 
                                     $table[] = $tbl[$f];
                                }
                            }
                        ...
                        if($strArray[$i] == 'where' || $strArray[$i] == 'WHERE'){
                             $strWhere = '';
                            for ($e = 0; $e < count($table) ; $e ++)
                            {
                                if(empty($table[$e]))
                                {
                                    continue;
                                }
                                 $strDelete =  $table[$e].".is_delete = 0";
                                if($e == 0)
                                {
                                    $strWhere =  $strDelete;
                                }
                                else
                                {
                                    ...
                                    {
                                        $strWhere = $strWhere." AND ".$strDelete;
                                    }
                                }
                            }
                           $strArray[$i] = "WHERE (".$strWhere.") AND";
                       }
                       
                    }
                    $sql = implode(' ',$strArray);
            }
            else
            {
                $e = 0;
                   for ($i = 0; $i < count($strArray) ; $i ++)
                    {
                        if($strArray[$i] == 'from' || $strArray[$i] == 'FROM')
                        {
                            $strtable = $strArray[$i+1];
                            if(strpos($strtable,',')!== false)
                                $table[] = explode(",",$strtable);
                            else
                                $table[] = $strtable;
                        }
                        if($strArray[$i] == 'group' || $strArray[$i] == 'GROUP')
                        {
                            $e = $i;
                        }
                    }
                    ...
                    if($e != 0)
                    {
                       $strArray[$e] = " WHERE (".$strWhere.") GROUP";
                       $sql = implode(' ',$strArray);
                    }
                    else
                    {
    ...
    public function delete($table, $where = '')
    {
         ...
         */
        //Khong xu dung delete ma update is_delete
