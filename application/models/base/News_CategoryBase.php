...
/**
 * Description of News_CategoryBase
 *
 * @author Vu
 */
class News_CategoryBase {
    //put your code here
    protected $_id;
    protected $_name;
    protected $_created;
...
class Model_DbTable_News_Category extends Zend_Db_Table_Abstract{
    protected $_name="news_category";
    protected $_primary="id";
}
class Model_News_MapperBase extends BaseMapper{
    protected $_dbTable;
    ...
    public function getDbTable()
    {
        if (NULL === $this->_dbTable) {
            $this->setDbTable("Model_DbTable_News_Category");
        }
        return $this->_dbTable;
    }
     public function save(Model_News_Category $entry){
        $data = array(
                'id' => $entry->getId(),
                'name'=>$entry->getName(),
    ...
    public function find($id, Model_News_Category $entry){
    ...
    public function fetchPaging($page, $pageSize, $orderBy, $orderDirection, &$totalRecord, $filters=null)
    {
		...
		$queryTotal = "SELECT count(*) as totals FROM news_category ".$sFilter."";
		$totalRecord = $this->getTotalRecords($queryTotal, $aFilterValues);
		$query = "SELECT * FROM news_category ".$sFilter." order by ".$orderBy." ".$orderDirection." limit ".($page-1)*$pageSize.",".$pageSize."";
