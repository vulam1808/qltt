 ...
 */
class Model_Master_Items_LimitBase{
    protected $_id;
    protected $_name;  
    protected $_code;    
    ...
    {   
        ...
        if (("mapper" == $name) || !method_exists($this, $method)) {   
            throw new Exception("Invalid Master_Items_Limit");   
        }   
        $this->$method($value);   
    }   
...
class Model_DbTable_Master_Items_Limit extends Zend_Db_Table_Abstract
{
    protected $_name = "master_items_limit";
    protected $_primary = "id";
}
class Model_Master_Items_LimitMapperBase extends BaseMapper{
     protected $_dbTable;
    public function setDbTable($dbTable)
    {
        ...
        if (!$dbTable instanceof Zend_Db_Table_Abstract) {
            throw new Exception("Invalid table data gateway Master_Items_Limited");
        }
    ...
    public function getDbTable()
    {
        if (NULL === $this->_dbTable) {
            $this->setDbTable("Model_DbTable_Master_Items_Limit");
        }
        return $this->_dbTable;
    }
    
    public function save(Model_Master_Items_Limit $entry){
        $data = array(
                'id' => $entry->getId(),
                'name'=> $entry->getName(),  
                'code'=>$entry->getCode(),
    ...
    public function find($id,  Model_Master_Items_Limit $entry){
     ...
     public function fetchPaging($page, $pageSize, $orderBy, $orderDirection, &$totalRecord, $filters=null)
    {
		...
		$queryTotal = "SELECT count(*) as totals FROM Master_Items_Limit ".$sFilter."";
		$totalRecord = $this->getTotalRecords($queryTotal, $aFilterValues);
		$query = "SELECT * FROM Master_Items_Limit ".$sFilter." order by ".$orderBy." ".$orderDirection." limit ".($page-1)*$pageSize.",".$pageSize."";
