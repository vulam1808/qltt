...
class Model_Master_PrintBase{
    ...
    {   
        ...
        if (("mapper" == $name) || !method_exists($this, $method)) {   
            throw new Exception("Invalid Master_Print");   
        }   
        $this->$method($value);   
    }   
...
class Model_Master_PrintMapperBase extends BaseMapper{
    ...
    public function setDbTable($dbTable)
    {
        ...
        if (!$dbTable instanceof Zend_Db_Table_Abstract) {
            throw new Exception("Invalid table data gateway Master_Print");
        }
    ...
    public function getDbTable()
    {
        if (NULL === $this->_dbTable) {
            $this->setDbTable("Model_DbTable_Master_Print");
        }
        return $this->_dbTable;
    }
    
    public function save(Model_Master_Print $entry){
        $data = array(
                'id' => $entry->getId(),
                'name'=> $entry->getName(),  
                'code'=>$entry->getCode(),
    ...
    public function find($id,  Model_Master_Print $entry){
     ...
     public function fetchPaging($page, $pageSize, $orderBy, $orderDirection, &$totalRecord, $filters=null)
    {
		...
		$queryTotal = "SELECT count(*) as totals FROM Master_Print ".$sFilter."";
		$totalRecord = $this->getTotalRecords($queryTotal, $aFilterValues);
		$query = "SELECT * FROM Master_Print ".$sFilter." order by ".$orderBy." ".$orderDirection." limit ".($page-1)*$pageSize.",".$pageSize."";
