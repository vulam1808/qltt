 ...
 */
class Model_Master_SanctionBase{
    protected $_id;
    protected $_name;  
    protected $_code;
    protected $_type;
    ...
    {   
        ...
        if (("mapper" == $name) || !method_exists($this, $method)) {   
            throw new Exception("Invalid master_sanction");   
        }   
        $this->$method($value);   
    }   
...
class Model_DbTable_Master_Sanction extends Zend_Db_Table_Abstract
{
    protected $_name = "master_sanction";
    protected $_primary = "id";
}
class Model_Master_SanctionMapperBase extends BaseMapper{
     protected $_dbTable;
    public function setDbTable($dbTable)
    {
        ...
        if (!$dbTable instanceof Zend_Db_Table_Abstract) {
            throw new Exception("Invalid table data gateway master_sanctioned");
        }
    ...
    public function getDbTable()
    {
        if (NULL === $this->_dbTable) {
            $this->setDbTable("Model_DbTable_Master_Sanction");
        }
        return $this->_dbTable;
    }
    
    public function save(Model_Master_Sanction $entry){
        $data = array(
                'id' => $entry->getId(),
                'name'=> $entry->getName(),  
                'code'=>$entry->getCode(),
    ...
    public function find($id,  Model_Master_Sanction $entry){
     ...
     public function fetchPaging($page, $pageSize, $orderBy, $orderDirection, &$totalRecord, $filters=null)
    {
		...
		$queryTotal = "SELECT count(*) as totals FROM Master_Sanction ".$sFilter."";
		$totalRecord = $this->getTotalRecords($queryTotal, $aFilterValues);
		$query = "SELECT * FROM Master_Sanction ".$sFilter." order by ".$orderBy." ".$orderDirection." limit ".($page-1)*$pageSize.",".$pageSize."";
