...
class Model_Admin_GroupBase{
    ...
    {   
        ...
        if (("mapper" == $name) || !method_exists($this, $method)) {   
            throw new Exception("Invalid Admin_Group property");   
        }   
        $this->$method($value);   
    }   
    ...
    public function getGroup_Name(){return $this->_group_name;}
...
class Model_DbTable_Admin_Group extends Zend_Db_Table_Abstract
{
    protected $_name = "admin_group";
    protected $_primary = "id";
}
class Model_Admin_GroupMapperBase extends BaseMapper{
     protected $_dbTable;
    ...
    public function getDbTable()
    {
        if (NULL === $this->_dbTable) {
            $this->setDbTable("Model_DbTable_Admin_Group");
        }
        return $this->_dbTable;
    }
    
    public function save(Model_Admin_Group $entry){
        $data= array(
            'id'=>$entry->getId(),
            'group_name'=>$entry->getGroup_Name(),
    ...
    public function find($id,  Model_Admin_Group $entry){
     ...
     public function fetchPaging($page, $pageSize, $orderBy, $orderDirection, &$totalRecord, $filters=null)
    {
		...
		$queryTotal = "SELECT count(*) as totals FROM admin_group ".$sFilter."";
		$totalRecord = $this->getTotalRecords($queryTotal, $aFilterValues);
		$query = "SELECT * FROM admin_group ".$sFilter." order by ".$orderBy." ".$orderDirection." limit ".($page-1)*$pageSize.",".$pageSize."";
    ...
    public function createCombo($id=0, $name='id', $width=230)
	{
		...
		foreach ($items as $item)
		{
			if($id == $item->getId())
			{
				$html .= '<option value="'.$item->getId().'"  selected="selected">'.$item->getGroup_Name().'</option>';
			}else{
				$html .= '<option value="'.$item->getId().'">'.$item->getGroup_Name().'</option>';
			}
		}
		$html .= '</select>';
		return $html;
