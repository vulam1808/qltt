 ...
 */
class DocDiaryController extends Zend_Controller_Action{
    ...
    public function addAction(){
        if($this->getRequest()->isPost()){
            $redirectUrl = $this->aConfig["site"]["url"]."default/docdiary/list";
            if(isset($_POST["id"])){
    ...
    public function serviceAction(){
        ...
        foreach ($this->modelMapper->fetchAll($userId) as $items ) {
            $menber[]=array(
                'Id'=>$items->getId(),
    ...
    public function servicetungaydenngayAction(){
        ...
        foreach ($this->modelMapper->fetchwhereAll($ngaybatdau,$ngayketthuc,$userId) as $items ) {
            $menber[]=array(
                'Id'=>$items->getId(),
                'date_diary'=>  GlobalLib::viewDate($items->getDate_Diary()),
                'implementers'=>$items->getImplementers(),                
...
//             $objPHPExcel->getActiveSheet()->getStyle('Q5:R9')->getFont()->setBold(true);
             ...
             $objPHPExcel->getActiveSheet()->getStyle('A10:'.'Q10')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
                ...
                $identity = Zend_Auth::getInstance()->getIdentity();
                $userId = $identity->id;
             foreach ($this->modelMapper->fetchwhereAll($tungay,$denngay,$userId) as $value ) {
