 ...
 */
class DocViolationsHandlingController extends Zend_Controller_Action{
    ...
    public function saveownerlessgoodsAction(){
        ...
        if($this->getRequest()->isPost()){
           $redirectUrl = $this->aConfig["site"]["url"]."default/docviolationshandling/listownerlessgoods";
           ...
           $this->modelMapper->updatesorderDoc_Violations_Handling();
            //them thong tin vao bang doc_violationshandling
                $this->model->setSys_Department_Id(17);
           ...
           for($i=0;$i<count($arraydataxuphatvochu);$i++){
                //luu du lieu vao doc_items_handling
                   $this->modelDocItemsHandling->setMaster_Items_Id((int)$arraydataxuphatvochu[0]['master_items_id']);
                   $this->modelDocItemsHandling->setMaster_Sanction_Id((int)$arraydataxuphatvochu[0]['master_sanction_id']);
                   $this->modelDocItemsHandling->setDoc_Violations_Handling_Id($id_doc_violationshandling);
                   $this->modelDocItemsHandling->setSerial_Handling($arraydataxuphatvochu[0]['serial_handling']);
                   $this->modelDocItemsHandling->setQuantity_Commodity($arraydataxuphatvochu[0]['quantity_commodity']);
                   $this->modelDocItemsHandling->setMaster_Unit_Id((int)$arraydataxuphatvochu[0]['master_unit_id']);
                   $this->modelDocItemsHandling->setDate_Handling(GlobalLib::toMysqlDateString($arraydataxuphatvochu[0]['date_handling']));
                   $this->modelDocItemsHandling->setAmount($arraydataxuphatvochu[0]['amount']);
                   $this->modelDocItemsHandling->setComment($arraydataxuphatvochu[0]['comment']);
                   $this->modelDocItemsHandling->setStatus($arraydataxuphatvochu[0]['status']); 
...
//            $this->redirect($redirectUrl);
   ...
   ///
    ...
    public function editshandlingAction() {
         $id = $this->_getParam("id","");
        $redirectUrl=$this->aConfig["site"]["url"]."default/docviolationshandling/list/type_business/DoanhNghiep";
        if(empty($id)){
            $this->_redirect($redirectUrl);
        }
...
//        }
    ...
    public function serviceownerlessgoodsAction(){
        ...
        echo json_encode($this->modelMapper->fetchvochu($status,$sys_department_id));
        exit();
    }
    ...
    public function servicerecordsmanagementgroupAction(){
      ...
      //foreach ($this->modelMapper->fetchAll() as $items ) {
        ...
        if($type == 'business')
        {
            ...
            {
                $where = $where." AND type_business = '".$typeBusiness."'";
            }
