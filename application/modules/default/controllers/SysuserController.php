 ...
 */
class SysUserController extends Zend_Controller_Action{
    ...
    public function editpassAction(){
        ...
        $redirectUrl=$this->aConfig["site"]["url"]."default/sysusergroup/list";
        if(empty($id)){
    ...
    public function addAction() {
        ...
        if($this->getRequest()->isPost()){
            $redirectUrl = $this->aConfig["site"]["url"]."default/sysuser/list";            
            if(isset($_POST["id"])){
...
//        if($popup == "oppent"){
//            $this->view->mo="opent";
//            $redirectUrl=$this->aConfig["site"]["url"]."default/sysuser/popup/id/".$id;
//             $this->_redirect($redirectUrl);
//        }
         $this->view->itemdepartment= $this->modelDepartment;
        $redirectUrl=$this->aConfig["site"]["url"]."default/sysuser/edit/id/$id";
        if(empty($id)){
            $this->_redirect($redirectUrl);
        }
        $this->modelMapper->find($id,$this->model);
        ...
        $this->view->districtHTML = GlobalLib::getComboByDistrict('master_district_id', 'master_district', 'id', 'name', $getDistrictId, false, 'form-control', '', $where1, '', 'onchange="getWardWithDistrict(\''.$this->aConfig["site"]["url"].'default/service/index'.'\')"');
        $this->view->wardHTML = GlobalLib::getComboByWard('master_ward_id', 'master_ward', 'id', 'name', $getWardId, false, 'form-control', '', $where, '', '');
        
//        $this->view->provinceHTML = GlobalLib::getComboByProvince('master_province_id', 'master_province', 'id', 'name', $getProvinceId, false, 'form-control', '', '', '', 'onchange="getDistrict1(\'' . $this->aConfig["site"]["url"] . 'default/service/index\',$(\'#master_province_id\').val(),\'master_district_id\',\'Xin Chờ Trong Giây Lát\');"');
//        $this->view->districtHTML = GlobalLib::getComboByDistrict('master_district_id', 'master_district', 'id', 'name',  $getDistrictId, false, 'form-control', '', 'where master_province_id=0', '','onchange="getWard1(\'' . $this->aConfig["site"]["url"] . 'default/service/index\',$(\'#master_district_id\').val(),\'master_ward_id\',\'Xin Chờ Trong Giây Lát\');"');
