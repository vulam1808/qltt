<?php
include APPLICATION_PATH . "/models/Master_Business_Type.php";
include APPLICATION_PATH . "/models/InfoBusiness.php";
/* 
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
class Master_MasterBusinessTypeController extends Zend_Controller_Action{
    public function init(){       
        $bootstrap = $this->getInvokeArg("bootstrap");
        $this->aConfig = $bootstrap->getOptions();
        $this->view->aConfig = $this->aConfig;
        $this->modelMapper= new Model_Master_Business_TypeMapper();
        $this->model= new Model_Master_Business_Type();
         $this->modelInfoBusiness= new Model_InfoBusiness();
        $this->modelMapperInfoBusiness= new Model_InfoBusinessMapper();
    }
    public function indexAction(){      
    }
    public function addAction(){
        if($this->getRequest()->isPost()){
            $redirectUrl = $this->aConfig["site"]["url"]."master/masterbusinesstype/list";
            if(isset($_POST["id"])){
    ...
    public  function confirmdeleteAction()
    {
       if($this->getRequest()->isPost()){
           ...
           $id_InfoBusiness = $this->modelMapperInfoBusiness->findidbyname1('master_business_type_id',$getId);
    ...
    public function deleteAction(){
        $id= $this->_getParam("id","");
        $redirectUrl=$this->aConfig["site"]["url"]."master/masterbusinesstype/list";               
        $this->modelMapper->deleteMaster_Business_Type($id);
        $this->_redirect($redirectUrl);
    }    
