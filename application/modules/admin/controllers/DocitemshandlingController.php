...
class Admin_DocItemsHandlingController extends Zend_Controller_Action{
 public function init(){
       ...
       $type=$this->_getParam("type","");  
        ...
        if(empty($id)){
         ...
         if($this->getRequest()->isPost()){
            $redirectUrl = $this->aConfig["site"]["url"]."admin/docitemshandling/list";            
  ...
  public function serviceAction(){
      ...
      $this->_helper->layout->disableLayout();
        foreach ($this->modelMapper->fetchAll($status,$month,$year) as $items ){
        $menber[] = array(
                'id' => $items->getId(),
                ...
                'doc_violations_handling_id'=> GlobalLib::getName("info_business",GlobalLib::getName("info_business",$items->getDoc_Violations_Handling_Id(),"id"),"name"),
  ...
  public function servicetichthuAction(){
      ...
      $year=$this->_getParam("year","");
      $this->_helper->layout->disableLayout();
        foreach ($this->modelMapper->fetchAlltichthu($status,$month,$year) as $items ){
        $menber[] = array(
                'id' => $items->getId(),
                'master_items_id'=> GlobalLib::getName("master_items",$items->getMaster_Items_Id(),"name"),  
                'master_sanction_id'=>  GlobalLib::getName("master_sanction",$items->getMaster_Sanction_Id(),"name"),
   ...
   public function deleteAction(){
        $id= $this->_getParam("id","");
        $redirectUrl=$this->aConfig["site"]["url"]."admin/docitemshandling/list";               
        $this->modelMapper->deleteDoc_Items_Handling($id);
        $this->_redirect($redirectUrl);
    }  
    public function deletetttAction(){
        $id= $this->_getParam("id","");
        $redirectUrl=$this->aConfig["site"]["url"]."admin/docitemshandling/listtt";               
        $this->modelMapper->deleteDoc_Items_Handling($id);
        $this->_redirect($redirectUrl);
    } 
