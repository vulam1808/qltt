...
class Admin_InfoScheduleController extends Zend_Controller_Action{
...
//        $this->modelDocViolationsHandling= new Model_Doc_Violations_Handling();  
    ...
    public function addAction(){
        $this->view->provinceHTML = GlobalLib::getComboByProvince('master_province_id', 'master_province', 'id', 'name', 0, false, 'form-control', '', '', '', 'onchange="getDistrictWithProvinceSchedule(\'' . $this->aConfig["site"]["url"] .'admin/service/index'. '\')"');
        $this->view->districtHTML = GlobalLib::getComboByDistrict('master_district_id', 'master_district', 'id', 'name', 0, true, 'form-control', '', 'where master_province_id=0', '', 'onchange="getWardWithDistrictSchedule(\''.$this->aConfig["site"]["url"].'admin/service/index'.'\')"');
        ...
        if($this->getRequest()->isPost()){
            $redirectUrl = $this->aConfig["site"]["url"]."admin/infoschedule/list/";            
            if(isset($_POST["id"])){
    ...
    public function editAction(){
        ...
        $this->view->districtHTML = GlobalLib::getComboCheckk('master_district_id', 'master_district', 'id', 'name', $getDistrictId, true, 'form-control', '', '', '', 'onchange="getWardWithDistrictSchedule(\''.$this->aConfig["site"]["url"].'admin/service/index'.'\')"');
